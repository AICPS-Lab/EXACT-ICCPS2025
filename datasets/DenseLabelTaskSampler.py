...
class DenseLabelTaskSampler(Sampler):
    ...
    def __init__(self, dataset: Dataset, n_way: int, n_shot: int, batch_size: int, n_query: int, n_tasks: int, threshold_ratio: float):
        """
        Args:
        ...
        self.threshold_ratio = threshold_ratio  # Ratio for non-background labels
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        for _ in range(self.n_tasks):
            sampled_task_indices = torch.cat([
                torch.tensor(random.sample(self.items_per_label[label], (self.n_shot + self.n_query) * self.batch_size))
    ...
    def episodic_collate_fn(
        ...
        """
        true_class_ids = list({x[1].max().item() for x in input_data})  # Use max value to identify class IDs in dense labels
        all_images = torch.cat([x[0].unsqueeze(0) for x in input_data])  # Stack all input samples
        all_labels = torch.cat([x[1].unsqueeze(0) for x in input_data])  # Stack all dense labels
        
        all_images = all_images.reshape(
            (self.n_way, self.n_shot + self.n_query, self.batch_size, *all_images.shape[1:])
        )
        all_labels = all_labels.reshape(
            (self.n_way, self.n_shot + self.n_query, self.batch_size, *all_labels.shape[1:])
        )

        # Separate support and query sets
        support_images = all_images[:, :self.n_shot].reshape((-1, *all_images.shape[3:]))
        query_images = all_images[:, self.n_shot:].reshape((-1, *all_images.shape[3:]))
        
        support_labels = all_labels[:, :self.n_shot].reshape((-1, *all_labels.shape[3:]))
        query_labels = all_labels[:, self.n_shot:].reshape((-1, *all_labels.shape[3:]))
