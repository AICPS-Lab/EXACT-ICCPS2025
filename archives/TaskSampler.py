...
class TaskSampler(Sampler):
    ...
    def __init__(self, dataset: Dataset, allowed_label: List[int], n_way: int, n_shot: int, batch_size: int, n_query: int, n_tasks: int):
        """
        In tasksampler have a param (allowed_label); e.g., class = [0,1,2,3,4], allowed_label = [1,2], therefore, n_ways can only be ≤ 2 bc len(allowed_label), 
        ...
        assert allowed_label != [], "allowed_label: the length of fg_allowed_labellabel should be greater than 0"
        assert all([label in self.items_per_label.keys() for label in allowed_label]), f"allowed_label: {allowed_label} should be in the dataset labels: {self.items_per_label.keys()}"

        assert len(allowed_label) >= n_way, f"allowed_label: the length of {allowed_label} should be greater than or equal to n_way: {n_way}"
    ...
    def episodic_collate_fn(
        ...
        # if it is in allowed_label, then it is its label, otherwise, it is 0 (background)
        ...
        # offset = 1 if self.with_bg_mode else 0
        ...
        support_labels = all_labels[:, : self.n_shot,]
        query_labels = all_labels[:, self.n_shot:,]
