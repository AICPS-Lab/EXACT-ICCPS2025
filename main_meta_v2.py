...
from loss_fn import dice_coefficient_time_series, euclidean_distance_time_series, iou_time_series
    ...
    for batch_idx in range(n_train_iter):
        ...
        for i in range(task_num):
            with higher.innerloop_ctx(
                ...
                Dice_score = dice_coefficient_time_series(softmax_qry, y_qry[i].long())
...
if __name__== "__main__":
    ...
    train_sampler = DenseLabelTaskSampler(
            train_dataset, n_way=2, n_shot=5, batch_size=32, n_query=5, n_tasks=train_dataset.NUM_TASKS, threshold_ratio=.25
        )
    test_sampler = DenseLabelTaskSampler(
            test_dataset, n_way=2, n_shot=5, batch_size=32, n_query=5, n_tasks=test_dataset.NUM_TASKS, threshold_ratio=.25
        )
