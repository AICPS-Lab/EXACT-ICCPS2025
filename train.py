import math
import os
from methods import Segmenter, TransformerModel
...
def main():
    ...
    model = TransformerModel(embed_dims=256, num_classes=5).float().to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), f'./saved_model/opportunity_transformer.pth')
        counter_i += 1
    # Test the model
    model.eval()
